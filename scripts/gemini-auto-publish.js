...
async function generateSingleNewsSection(secConfig, dateInfo) {
  ...
  if (candidates.length === 0) {
    console.warn(`  ⚠️ [${secConfig.category}] 48시간 이내 후보 기사가 0건입니다.`);
  ...
  const candidateListText = candidates
    .map((c, i) => {
      ...
      return `${i}. [${c.source}] ${c.title}${snippetPart}`;
    })
    .join('\n');

const prompt = `
당신은 엄격한 팩트 기반의 전문 뉴스 요약 에디터입니다.
아래 제공된 "후보 기사 목록"에서 해당 분야의 가장 중요한 핵심 기사를 최대 5개 선택하여 한 문장으로 요약하십시오.
...
### [작성 규칙 및 팩트 보존 원칙 - 엄격 준수]
1. [철저한 원문 종속]: 반드시 각 후보 항목의 [제목]과 [스니펫]에 **실제로 적혀 있는 단어, 고유명사, 수치만** 사용하여 요약하십시오.
2. [환각 및 추정 절대 금지]:
   - 원문에 없는 원인, 배경, 향후 전망, 개인적 추정을 **단 한 단어도 추가하지 마십시오**.
   - 영문 기사인 경우에도 원문의 의미를 정확히 직역/요약하되, 모델 자체의 사전 지식을 보태지 마십시오.
   - 스니펫이 없거나 짧은 항목은 억지로 내용을 늘리지 말고, 제공된 사실만으로 간결하게 작성하십시오.
3. [문장 구조]:
   - [주체/기관] + ",(쉼표) " + [원문에 명시된 구체적 조치 및 수치]가 드러나도록 작성하십시오.
   - 문장 끝은 기존 스타일대로 명사형/체언 종결(~판결, ~결정, ~서명, ~승인, ~발표, ~논란, ~기록 등)로 깔끔하게 맺으십시오.
4. source나 부가 설명 없이 JSON 형식으로만 응답하십시오.
...
반드시 아래 JSON 형식으로만 응답하세요:
{
  "picks": [
    {
      "index": 0,
      "text": "원문의 팩트와 수치만을 충실히 조합한 명사형 종결 문장"
    }
  ]
}`;
...
// 8. [주식 모닝 브리핑] 시스템 프롬프트 (정통 증시 시황 10단계 구성 + 확정 수치 연동)
function getStockSystemPrompt(dateInfo, marketData) {
  ...
  return `
당신은 매일 아침 글로벌 매크로 지표, 빅테크 실적, 원자재, 그리고 국내 증시 수급과 기술적 분석을 총망라하여 브리핑하는 최고 권위의 증시 수석 전략가입니다.
아래 제공된 [Yahoo Finance 실시간 실제 수치]를 100% 확정 팩트로 활용하고, Google Search로 확인된 밤사이 글로벌 시황 및 직전 거래일 국내 마감 팩트를 결합하여 "10단계 주식 모닝 브리핑"을 작성하세요.
...
2. 문체 규칙: 문장 종결은 반드시 "~함.", "~임.", "~있음.", "~없음.", "~기록함.", "~상회함.", "~하회함.", "~부합함." 스타일의 간결한 개조식 서술형으로 통일할 것.
3. weather 필드: 글로벌 시장 분위기 및 핵심 테마 1줄 압축 요약.
4. highlights 필드: 당일 장 시작 전 반드시 확인해야 할 3대 핵심 포인트 (~함/임 종결).
...
### [10단계 고정 섹션 구성 (순서 및 ID 엄수)]

- **1단계 (id: "sec_1", category: "1. 미 증시 마감 요약 및 오프닝", icon: "TrendingUp", items: 4개)**:
  * 다우, S&P500, 나스닥, 러셀2000 등 뉴욕 증시 전반의 흐름, 지수별 차별화 원인, 장중 변동성 요인 총괄 요약.
  * source: "해외 증시 요약"

- **2단계 (id: "sec_2", category: "2. 주요 경제 지표 및 글로벌 뉴스 점검", icon: "Globe", items: 4개)**:
  * 밤사이 발표된 거시 지표(GDP, 고용, 소비 등), 중앙은행 발언, 지정학적 이슈(중동/러시아/양안 등) 핵심 뉴스.
  * source: 실제 언론사명 (로이터, 블룸버그, WSJ 등)

- **3단계 (id: "sec_3", category: "3. 핵심 거시 지표 상세 분석", icon: "BarChart3", items: 4개)**:
  * 물가(CPI/PCE/PPI), 미국 10년물 국채금리 변동 추이, 달러 인덱스 및 NDF 원/달러 환율 방향성이 자산시장에 미친 영향 상세 분석.
  * source: "매크로 지표 분석"

- **4단계 (id: "sec_4", category: "4. 글로벌 빅테크 및 반도체 실적/이슈 분석", icon: "Cpu", items: 4개)**:
  * 엔비디아, 애플, TSMC, 마이크론 등 주요 빅테크/반도체 기업의 실적 발표 수치(매출, EPS, 가이던스) 또는 주요 코멘트/이슈 집중 분석.
  * source: "빅테크/반도체"

- **5단계 (id: "sec_5", category: "5. AI 생태계 및 주요 산업 투자 동향", icon: "Sparkles", items: 3개)**:
  * 하이퍼스케일러들의 AI 데이터센터 설비투자(CapEx), AI 소프트웨어 수익화 모델, 전력/냉각/원전 등 AI 인프라 연관 산업 흐름 분석.
  * source: "AI 산업 동향"

- **6단계 (id: "sec_6", category: "6. 해외 주요 특징주 및 원자재 시장 동향", icon: "Zap", items: 4개)**:
  * 당일 호재/악재로 급등락한 개별 특징주 분석 + 국제유가(WTI), 구리, 금 등 핵심 원자재 가격 등락 원인.
  * source: "특징주/원자재"

- **7단계 (id: "sec_7", category: "7. 국내 증시(코스피/코스닥) 및 대표주 흐름 점검", icon: "LineChart", items: 4개)**:
  * ${weekendTag}국내 증시 기술적 지지/저항선, 코스피·코스닥 거래대금 추이, 삼성전자·SK하이닉스 등 시총 상위 대표주 기술적 흐름.
  * source: "국내 증시 점검"

- **8단계 (id: "sec_8", category: "8. 외국인 및 기관 수급/업종 쏠림 분석", icon: "Users", items: 3개)**:
  * 직전 정규장 기준 외국인/기관의 현·선물 매매 포지션, 특정 업종으로의 자금 쏠림 또는 이탈 현상 분석.
  * source: "시장 수급 분석"

- **9단계 (id: "sec_9", category: "9. 큰손 집중 매수 종목군 및 특징", icon: "Target", items: 3개)**:
  * 외국인 및 기관 동반 순매수 상위 종목군 및 연속 매수세가 유입되는 테마/섹터의 공통적 특징 정리.
  * source: "메이저 수급 특징주"

- **10단계 (id: "sec_10", category: "10. 시장 종합 시사점 및 투자 전략 정리", icon: "Compass", items: 4개)**:
  * [글로벌 환경의 국내 영향 평가], [당일 장 개장 전 관전 포인트], [주목 유망 섹터/업종], [단기 및 스윙 실전 대응 전략].
  * source: "실전 투자 전략"
...
반드시 다른 설명 없이 JSON 구조의 \`\`\`json ... \`\`\` 블록으로만 응답하세요:
{
  ...
  "sections": [
    {
      "id": "sec_1",
      "category": "1. 미 증시 마감 요약 및 오프닝",
      "icon": "TrendingUp",
      "items": [{ "text": "...", "source": "해외 증시 요약" }]
    }
  ]
}
`;
