...
2. 실시간 팩트 검증 (최우선):
   - 반드시 오늘(${dateInfo.isoDate}) 기준 최근 24시간 이내에 실제 보도된 사실만 작성할 것.
...
6. 8대 섹션 구성 (각 섹션 정확히 5개 항목 구성):
   ...
   - 섹션 8 (id: "sec_8", category: "[스포츠:이정후.안세영.KLPGA.LPBA]", icon: "Sparkles", items: 5개)
     * 스포츠 섹션 필수 5개 항목 매핑 규칙:
       1) 이정후 (최근 공식 경기 활약/기록/소식)
       2) 안세영 (최근 배드민턴 대회 성적/협회/근황 소식)
       3) KLPGA (현재 실제 진행 중인 투어 대회 실명 및 주요 선수 순위/소식)
       4) LPBA (현재 실제 진행 중인 PBA-LPBA 투어 실명 및 경기 결과/소식)
       5) 당일 대한민국 스포츠 핫이슈 (손흥민, KBO 프로야구, 김하성, 배준호 등 실제 오늘자 톱 스포츠 뉴스 1건)
       ※ 만약 해당 선수의 당일 경기 소식이 없을 경우, 억지로 가상 사실을 지어내지 말고 최근 공식 인터뷰/순위/근황 또는 오늘자 대한민국 핵심 스포츠 스타 뉴스로 대체할 것.
7. 항목(item) 작성 규칙:
...
5. 전체 4대 섹션 구성 및 순서 (순서 변경 불가):
   - 섹션 1 (id: "sec_1", category: "1. 해외 증시 마감 현황", icon: "TrendingUp"):
     다우 지수, S&P 500, 나스닥, 러셀 2000, 필라델피아 반도체 지수, MSCI 한국 지수 ETF, 야간 선물 등 7대 지수의 실제 마감 수치와 등락률(%), 원인 및 마감 동향을 1줄로 명시.
     * source는 각각 "다우", "S&P500", "나스닥", "소형주", "반도체", "한국물", "선물"로 지정.
   - 섹션 2 (id: "sec_2", category: "2. 오늘의 증시 키워드", icon: "TrendingUp"):
     당일 글로벌/국내 시장을 관통하는 실제 핵심 테마 및 이슈 4가지 (~함/임 종결, source: "핵심 키워드")
   - 섹션 3 (id: "sec_3", category: "3. 주요 주식 뉴스", icon: "TrendingUp"):
     오늘자 시장 영향력이 큰 핵심 뉴스 4개. text 형식은 "[헤드라인]: 시장 영향 요약 설명" 형태로 작성하고, source에는 실제 출처 언론사 명시
...
async function publishBriefing(categoryType) {
  ...
  const userPrompt = isStock
    ? `Google Search를 사용하여 ${dateInfo.isoDate} 오늘 기준 미국 증시 3대 지수 실제 마감 수치와 최신 금융 뉴스를 검색하고 [주식 모닝 브리핑] JSON 데이터를 생성하세요.`
    : isInsight
    ? `20대 청년을 위한 깊이 있는 주제를 바탕으로 [생각의 원점] 고전/명저 인용(3~4문장)과 [마인드 피벗] 정중한 경어체(~합니다) 실천 해설(3~4문장)을 담은 [데일리 인사이트] JSON 데이터를 생성하세요.`
